...
def checkChunk(data):
    pos = (int((getCameraPosition() * Vec3(1, 0, 0)).x // 8) - 1, int((getCameraPosition() * Vec3(0, 0, 1)).z // 8) - 1, int(round((getCameraPosition() * Vec3(0, 1, 0)).y + 15) / 5) + 1)
    newWaitingChunk = []
    for x in range(pos[0] - 2, pos[0] + pos[2] * rotations[rotationNumber][2][0], rotations[rotationNumber][2][0]):
        for y in range(pos[1] - 2, pos[1] + pos[2] * rotations[rotationNumber][2][1], rotations[rotationNumber][2][1]):
